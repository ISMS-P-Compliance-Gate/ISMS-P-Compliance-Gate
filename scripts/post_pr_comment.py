...
필요 패키지: pip install google-generativeai pyyaml requests
"""
...
from lib.remediation import generate_comment_lines  # noqa: E402
...
def collect_latest_results() -> list:
    ...
    예시로 아래 두 구조 모두 지원한다.
      results/2_8_4/2026-07-23T20-55-51Z.json          (폴더로 항목 구분)
      results/2_8_4_result_20260723_205551.json         (평평한 구조, 파일명에 항목 포함)
    ...
    all_json_paths = glob.glob(os.path.join(RESULTS_DIR, "**", "*.json"), recursive=True)

    latest_by_control = {}  # control_id -> (timestamp_str, dict)
...
def build_comment_markdown(results: list, use_llm: bool = True) -> str:
    ...
    for r in results:
        icon = STATUS_ICON.get(r.get("status"), "❔")
        lines.append(f"\n**{icon} {r.get('control_id')} — {r.get('control_name')}**")
        for sentence in generate_comment_lines(r, use_llm=use_llm):
...
def _running_in_github_actions() -> bool:
    return bool(os.environ.get("GITHUB_ACTIONS")) and bool(os.environ.get("GITHUB_TOKEN"))
...
def post_to_github(comment_body: str):
    ...
    if not event_path or not os.path.exists(event_path):
        raise RuntimeError("GITHUB_EVENT_PATH 를 찾을 수 없습니다. PR 이벤트에서만 실행 가능합니다.")
    ...
    page = 1
    while True:
        resp = requests.get(base_url, headers=headers, params={"per_page": 100, "page": page})
    ...
    if existing_comment_id:
        ...
        resp = requests.patch(url, headers=headers, json={"body": comment_body})
    else:
        resp = requests.post(base_url, headers=headers, json={"body": comment_body})

    resp.raise_for_status()
    print(f"PR #{pr_number} 코멘트 게시 완료 (status={resp.status_code})")
...
def main():
    ...
    parser.add_argument(
        ...
        help="GitHub에 실제로 올리지 않고 로컬 파일(pr_comment_preview.md)로만 저장",
    )
    parser.add_argument(
        "--no-llm", action="store_true",
        help="Gemini 호출 없이 템플릿 문장만으로 생성 (빠른 구조 확인용)",
    )
    args = parser.parse_args()

    results = collect_latest_results()
    if not results:
        print(f"results/ 에서 점검 결과를 찾지 못했습니다: {RESULTS_DIR}")
        return

    comment_body = build_comment_markdown(results, use_llm=not args.no_llm)

    if args.dry_run or not _running_in_github_actions():
        out_path = os.path.join(BASE_DIR, "pr_comment_preview.md")
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(comment_body)
        print(f"[dry-run] GitHub Actions 환경이 아니거나 --dry-run 지정됨 → {out_path} 에 저장했습니다.")
        print("\n----- 미리보기 -----\n")
        print(comment_body)
    else:
